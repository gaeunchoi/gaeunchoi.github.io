...
3. 분할 정복 알고리즘을 이용한 문제풀이
   - a^b  
   경우를 2가지로 나누어 풀었다.  
   b가 홀수일때 : $ a \times a^{b-1} $  
   b가 짝수일때 : $ a^{b/2} \times a^{b/2}$  
   여기서 b가 짝수일때 $a^{b/2}$를 두번 호출하는게 아니라 $a^{b/2}$결과값을 변수에 저장 후 그 변수를 곱해줘야 시간복잡도가 줄어들..걸..? ㅋㅋㅋㅋㅋ 내 생각은 그럼 잘 모름    
   ...
   ```java  
    ...
    - 가장 가까운 두 점  
   삽질의 끝. 알고리즘 생각했는데 하루종일 구현 못해서 멘탈 터졌음. 예시 다 만들어서 아이패드에서 하나하나 다 삽질했는데 구현에서 막히고 막혀서 상현오빠한테 헬프쳤더니 오빠가 3이하로는 다 노가다 시키고 그 이후로만 분할정복하면 코드짜기쉬울거라해서 그대로 짰더니 1시간도 안걸려 뚝-딱 ... 온전히 내힘으로 했다고 할 순 없는 문제풀이 .. 나는 언제 코딩 늘지 .. ?  
